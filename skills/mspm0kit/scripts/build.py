...
def _generate_makefile(ticlang_dir: Path, proj: Path, config: dict) -> None:
    ...
    # Collect all .c files in project root (flat structure)
    c_files = sorted([f.name for f in proj.glob("*.c")])
    if not c_files:
        raise FileNotFoundError("No .c source files found in project root")
    ...
    obj_files = [f"{Path(cf).stem}.o" for cf in c_files]

    # Generate makefile
    makefile = f"""# Auto-generated by build.py — minimal makefile for CLI builds
...
"""
    for cf in c_files:
        src_dir = "." if (ticlang_dir / cf).exists() else ".."
        makefile += f"{Path(cf).stem}.o: {src_dir}/{cf}\n"
        makefile += f"\t$(CC) $(CFLAGS) -c $< -o $@\n\n"
